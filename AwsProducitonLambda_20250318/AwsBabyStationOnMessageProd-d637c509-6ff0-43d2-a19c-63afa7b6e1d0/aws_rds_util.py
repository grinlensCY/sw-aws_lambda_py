...
ERROR_SLEEP_STAGE_PARSE=-30000
ERROR_SLEEP_STAGE_S3_UPLOAD=-30001
ERROR_SLEEP_STAGE_S3_DOWNLOAD=-30002
ERROR_SLEEP_STAGE_INSERT=-30003
ERROR_SLEEP_STAGE_GET=-30004
...
ERROR_SLEEP_STAGE_PARSE:"Sleep Stage資料轉換錯誤",
ERROR_SLEEP_STAGE_S3_UPLOAD:"Sleep Stage S3上傳失敗",
ERROR_SLEEP_STAGE_S3_DOWNLOAD:"Sleep Stage S3下載失敗",
ERROR_SLEEP_STAGE_INSERT:"無法新增Sleep Stage info到資料庫",
ERROR_SLEEP_STAGE_GET:"無法從資料庫讀取Sleep Stage info"
}
...
EVENT_STR_BS='bs'
EVENT_STR_WAKEUP='wakeup'
EVENT_STR_OBS='obs'
EVENT_STR_COMFORT='comfort'
...
EVENT_TYPE_WAKEUP:EVENT_STR_WAKEUP,
EVENT_TYPE_OBS:EVENT_STR_OBS,
EVENT_TYPE_GO_COMFORT:EVENT_STR_COMFORT
}
...
EVENT_STR_WAKEUP:EVENT_TYPE_WAKEUP,
EVENT_STR_OBS:EVENT_TYPE_OBS
}
...
def saveOddSnd(cognito_id,dev_udid,ts,ba):
    path='odd_snd/%s/%s/%d.bin'%(cognito_id,dev_udid,ts)

    try:
        bio=io.BytesIO(ba)
        s3.upload_fileobj(bio,S3_BUCKET, path)
    except:
        traceback.print_exc()
...
#==========================================================================
def saveSleepStage(cur,conn,cognito_id,bid,ts,map):#ts是以ms為單位
    #不檢查cognito_id與bid(baby id)的關聯性，因為這個指令只會被sta lambda處理，處理過程就會分析cognito_id與baby id

    VALUES=(SLEEP_STAGE_TABLE_NAME,bid,ts)
    STATE="INSERT INTO %s (baby_id,aws_ts) values (%d,%d)"%VALUES
    ...
    try:
        ...
        traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_INSERT)
    db_idx=cur.lastrowid

    try:
        cxt_str=json.dumps(map)
        cxt=cxt_str.encode('utf-8')
    except:
        traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_PARSE)

    try:
        bio=io.BytesIO(cxt)
        s3.upload_fileobj(bio,S3_BUCKET, 'sleep_stage/%s/%s/%d.json'%(cognito_id,bid,ts))
    except:
        traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_S3_UPLOAD)

    return (True,db_idx)

def readSleepStageInfo(cur,bid,bts,ets):
    LIMIT=16
    STATE="SELECT ss_id,aws_ts FROM %s WHERE baby_id=%d AND aws_ts BETWEEN %d AND %d ORDER BY aws_ts ASC LIMIT %d"%(SLEEP_STAGE_TABLE_NAME,bid,bts,ets,LIMIT)
    ...
    try:
        ...
        traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_GET)
...
def loadSleepStage(cognito_id,bid,ts):
    s3_path='sleep_stage/%s/%s/%d.json'%(cognito_id,bid,ts)
    ...
    try:
        ...
        #traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_S3_DOWNLOAD)

    try:
        bio.seek(0)
        cxt=bio.read()
        cxt_str=cxt.decode('utf-8')
        map=json.loads(cxt_str)
        return (True,map)
    except:
        traceback.print_exc()
        return (False,ERROR_SLEEP_STAGE_PARSE)
