...
def chk_alg():
    ...
    has_poo,has_obs=OSAA.checkRaw(ba,'test',0.0,3)
...
def odd_snd_proc(cur,conn,cognito_id,param):
    ...
    #save to s3
    res=aws_rds_util.saveOddSnd(cognito_id,dev_udid,esp_ts,ba)
    if(ba==None or len(ba)<10*1024 or res[0]==False):
        return False
    snd_s3_path=res[1]
    ...
    has_poo,has_obs=OSAA.checkRaw(ba,dev_udid,tick/32768.0,month)
