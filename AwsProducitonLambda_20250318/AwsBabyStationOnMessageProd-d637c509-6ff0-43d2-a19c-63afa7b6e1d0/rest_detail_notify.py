...
def handle_detail_notify(cur,body):
    ...
    info=CU.get_cache_data(key);
    if(info is None):
        notify_list.append({'type':RN.AWS_NOTIFY_COMFORT_STATE,'ts':cts,'state':False})
        pass
    elif(cts-(info['ts'])<max_alarm_time):
        notify_list.append(info)
    
    #for test    
    #if(sta_udid=='SX_BABY_STA_0000'):
    #    notify_list.append({'type':RN.AWS_NOTIFY_COMFORT_STATE,'ts':cts,'state':True})
        #notify_list.append({'type':RN.AWS_NOTIFY_COMFORT_STATE,'ts':cts,'state':True,'go_comfort_ts':777,'is_awake':False,'is_awake_ts':888})
