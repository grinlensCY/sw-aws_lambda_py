...
class SleepStatus():
    def __init__(self, udid, age, ver=20240401, scaled=True):
        ...
        self.sleep_stat_stillCnt_th = (3.5 if self.age > 6 else 1.5)*60*self.imusr//scale  # 睡眠狀態的stillCnt門檻
        self.lightsleep_stat_stillCnt_th = (8 if self.age > 6 else 6)*60*self.imusr//scale
        ...
        self.storage_sec = 50*60  # 50min之前的realtime資料就不要了
        # self.status_storage_len = 5*60//self.broadcast_intvl  # 啟動安撫要間隔上次啟動多少個訊息
        self.status_storage_sec = 5*60  # status_list 保留多久
        ...
        self.comfort_period_UL_sec = 10*60  # 每一次的comfort最多10min

        self.awakeAlarm_sleep_th_sec = 20*60    # 要連續睡眠多久才能啟動awakeAlarm
    ...
    def load_context(self,udid):
        ...
        if(res is None):
            ...
            'deep_cnt':0,
            'goComfort': False,
            'comfort_start_ts': 0,  # 用來限制最長的goComfort時間(10min)
            'last_deep_ts': 0,
            # awakeAlarm
            'last_awake_ts': 0, # 最後清醒的時間
            'awakeAlarm_ts': 0, # 發出alarm的時間
            'sleep_start_ts':0  # 為了確認是連續awakeAlarm_sleep_th_sec的睡眠時間(for awakeAlarm)
            }
        return res
    ...
    def clear_expired_data(self,ts,vars):
        '''
        remove expired status_list + status_ts_list
        reset BL if expired
        remove expired BL
        remove expired realtime data + hr_devi_chk_idx + rr_devi_chk_idx + BL_idx_chked
        '''
        # = 移除 超過status_storage_sec(5min) 的status
        idx = len(vars['status_ts_list'])
        for t in vars['status_ts_list'][::-1]:
            if ts - t > self.status_storage_sec:
                del vars['status_list'][:idx]
                del vars['status_ts_list'][:idx]
                break
            idx -= 1
        if not len(vars['status_ts_list']): # 太久沒收到資料, 不好判斷，就reset
            vars['sleep_start_ts'] = 0

        # = reset BL periodically (3 days)
        if vars['BL_ti'] and ts - vars['BL_ti'] > self.clear_BL_sec:
            self.resetBL(vars, 'expired')
        
        # = 移除超過storage_sec(50min)的realtime/BL(baseline) data
        while (len(vars["BL_timeslot"]) and ts-vars["BL_timeslot"][0][0] > self.storage_sec):
            #print(f'clear BL_timeslot[0], {vars["BL_timeslot"][0]}={self.hhmmss(vars["BL_timeslot"][0][0])} ~ {self.hhmmss(vars["BL_timeslot"][0][1])}')
    ...
    def get_status(self,vars,scdat,devi_dat,devi_rising_score_dat,ts):
        ...
        # limit longest goComfort status
