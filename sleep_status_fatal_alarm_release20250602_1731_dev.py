...
class SleepStatus_FatalAlarm():
    def __init__(self, age, ver=20250317.61, scaledSC=True):
        self.ver = ver
        self.imusr = 104
        ...
        self.ratio_to_pre_UL = [1.18, 1.15]    # hr/rr ratio UL   if ratio to pre_val > UL => skip this
        self.ratio_to_pre_LL = [0.85, 0.85]
        self.ratio_to_ref_UL = [1.65, 1.7]
        ...
        self.risingSoon_duration_th = {'hr': [20,50], 'rr': [35,50]}    # 定義快速上升的時間範圍
    ...
    def getBaseline(self,vars,tsData,lcf):
        ...
        while lcf < tsData.size:
            ...
            if len(vars['hr_baseline']) and vars['hr_baseline'][-1][2] != -1:
                ...
                #else:
                #    self.aMsg(f"hr: no data < 1.3X lastest baseline => skip this",1)
            else:
            ...
            if len(vars['rr_baseline']) and vars['rr_baseline'][-1][2] != -1:
                ...
                if mask.all():
                    ...
                    # self.aMsg(f"30th percentile={perc30}",2)
                #else:
                #    self.aMsg(f"rr: no data < 1.3X lastest baseline => skip this",1)
            else:
    ...
    def is_fatal(self,vars,ts,hrDat,rrDat,attached,status):
        ...
        if status is Sleep.NONE or not attached or vars['pre_ts'] is None or status is Sleep.AWAKE:
            #self.aMsg(f"\nis_fatal: ts={ts}={ts_str}  status={status} is None/AWAKE?  attached?{attached}  pre_ts?{vars['pre_ts']}  ts={ts} => quit and return False ")
            return False
        
        if vars['sleep_bts'] is None or ts - vars['sleep_bts'] < 1200:
            #msg = (f"\nis_fatal: ts={ts}={ts_str}")
        ...
        #self.aMsg(f"\nis_fatal: {ts}={ts-self.debugVars['t0']}={ts_str}\n")
        ...
        for dat_idx,info in enumerate(data.items()):
            typ,(val,cl) = info
            # if ts-self.debugVars['t0'] > 3580:
            #     self.aMsg(f"{typ},({val=},{cl=})  {vars[f'pre_{typ}']}")

            if cl < 5:  #clth:
                continue

            # 變化太大 不合理 => 跳過
            avg_last2 = (vars[f'pre_{typ}'] + vars[f'pre2_{typ}'])*0.5
            max_last2 = max(vars[f'pre_{typ}'], vars[f'pre2_{typ}'])
            if ((val >= avg_last2*self.ratio_to_pre_UL[dat_idx]) # 高於 前兩筆資料最小值 * ratio_to_pre_UL
                    or (val <= max_last2*self.ratio_to_pre_LL[dat_idx])):    # 低於 前兩筆資料最大值 * ratio_to_pre_LL
                #vars[f'last_{typ}_bigChange_ts'] = ts
                #self.aMsg(f"\nis_fatal: bigChange  {typ}({val}) >= "
                #          f"avg(pre({vars[f'pre_{typ}']}), pre2({vars[f'pre2_{typ}']}))*{self.ratio_to_pre_UL[dat_idx]}="
                        #   f"{avg_last2*self.ratio_to_pre_UL[dat_idx]:.2f}  OR  "
                        #   f"<= {max_last2*self.ratio_to_pre_LL[dat_idx]:.2f}",1)
            ...
            if not val:    # 不是有效的hr, rr
                ...
                if val < vars[f'pre_{typ}']:    # falling
                    vars[f'same_{typ}_duration'] = 0
                    ...
                    if val <= vars[f'lastest_{typ}_baseline']:
                    ...
                    else:   # > lasteset baseline 確認是不是有 像pulse的狀況(快速起伏)
                        vars[f"{typ}_fallingTime"] = 0
                        if vars[f"{typ}_isRisingSoon"]:
                            ...
                            # self.aMsg(f"{vars[f'{typ}_overUL_ts']=}  {self.risingSoon_duration_th[typ][0]} <= {vars[f'{typ}_risingTime']=} <= {self.risingSoon_duration_th[typ][1]}",2)
                            if (vars[f'{typ}_overUL_ts']
                                ...
                                # self.aMsg(f"update {typ}_isRisingSoon")
                            else:
                                vars[f'{typ}_overUL_ts'] = 0
                            vars[f"{typ}_risingTime"] = 0
                elif val > vars[f'pre_{typ}']:  # rising
                    vars[f"{typ}_fallingTime"] = vars[f'same_{typ}_duration'] = 0

                    # self.aMsg(f"{val=} > {f'lastest_{typ}_baseline'}={vars[f'lastest_{typ}_baseline']}?",2)
                    if val <= vars[f'lastest_{typ}_baseline']:
                        vars[f"{typ}_risingTime"] = vars[f"{typ}_overUL_ts"] = 0
                    else:   # 高於baseline的才有意義
                        vars[f"{typ}_risingTime"] += intvl
                        # self.aMsg(f'{val=} >= {f"{typ}_baseline_alarm_UL"}={vars[f"{typ}_baseline_alarm_UL"]:.1f}?',3)
                        if val >= vars[f"{typ}_baseline_alarm_UL"]:
                            vars[f'{typ}_overUL_ts'] = ts
                            # self.aMsg(f"update {f'{typ}_overUL_ts'} to {ts}=>{ts_str}",4)
                        else:
                            vars[f'{typ}_overUL_ts'] = 0
                else:
                    vars[f'same_{typ}_duration'] += intvl
                    if vars[f'same_{typ}_duration'] > self.one_update_intvl[typ]:
                        # self.aMsg(f"{vars[f'same_{typ}_duration']=} > {self.one_update_intvl[typ]} ==> reset_isfatel_tmp_vars",2)
                        self.reset_isfatel_tmp_vars(vars,[typ])
